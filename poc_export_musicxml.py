...
def key_fifths(tonic_pc, mode):
    ...
    return MAJOR_FIFTHS[major_tonic_pc]
...
def build_musicxml(notes_path, audio_path, title, override_bpm=None):
    ...
    for m in sorted(quantized_by_measure.keys()):
        ...
        measures_xml.append(f'<measure number="{m}">{attrs_xml}{"".join(notes_xml)}</measure>')
    ...
    return xml, {"tempo": tempo, "key": f"{PITCH_CLASS_NAMES[tonic]} {mode}", "measures": len(measures_xml)}
