...
## 8. 회귀 테스트 (파이프라인 코드를 고쳤을 때 항상 먼저 돌릴 것)

`poc_*.py`(스케일필터/운지최적화/리듬정렬/렌더링) 어디든 코드를 수정했다면, 새 곡
결과를 사용자에게 보여주기 전에 먼저 프로젝트 루트에서:

```bash
python tests/regression/run.py
```

를 돌려서 이미 검증 끝난 예전 곡(나는나비·돈룩백)의 결과가 그대로 유지되는지 확인한다.
실패하면 지금 수정한 코드가 예전에 검증된 결과를 깨뜨렸다는 뜻이니, 새 곡 작업을
계속하기 전에 원인부터 파악할 것. 자세한 설계 배경은 `tests/regression/README.md` 참고.
